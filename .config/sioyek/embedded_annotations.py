...
PATH_TO_SIOYEK = r'/usr/bin/sioyek'
...
def add_highlight(doc_path, page_number, text):
	doc = fitz.open(doc_path)
	page = doc.load_page(page_number)
	quads = page.search_for(text, flags=fitz.TEXT_PRESERVE_WHITESPACE, hit_max=50)
...
if __name__ == '__main__':
	if sys.argv[1] == 'bookmark':
		pos1 = sys.argv[4].split(" ")[1]
		pos2 = sys.argv[4].split(" ")[2]
		add_bookmark(sys.argv[2], int(sys.argv[3]), (float(pos1), float(pos2)), sys.argv[5])
